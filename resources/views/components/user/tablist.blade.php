<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
    <div class="p-6 bg-white">
        ...
        <section class="tab-content hidden" id="favorite">
            @foreach ($favoriteTweets as $favorite)
                <div class="bg-white px-4 py-5 sm:px-6 border-b border-gray-200 pb-5">
                    <div class="flex space-x-4">
                        <div class="relative flex-shrink-0">
                            <img class="h-10 w-10 rounded-full" src="{{ asset('storage/' . $favorite->user->image) }}" alt="">
                            @if ($favorite->user->isOnline())
                                <span class="absolute right-0 top-0 block h-2.5 w-2.5 rounded-full bg-green-400 ring-2 ring-white"></span>
                            @else
                                <span class="absolute right-0 top-0 block h-2 w-2 rounded-full bg-red-400 ring-2 ring-white"></span>
                            @endif
                        ...
                        <div class="min-w-0 flex-1">
                            <div class="flex justify-between">
                                <div>
                                    <p class="text-sm font-semibold text-gray-900">
                                        <a href="{{ route('users.show', ['user' => $favorite->user_id]) }}" class="hover:underline">{{ $favorite->user->name }}</a>
                                    </p>
                                    <p class="text-sm text-gray-500">
                                        <a href="#" class="hover:underline">{{ $favorite->created_at->diffForHumans() }}</a>
                                    </p>
                                </div>
                            </div>
                            <a href="{{ route('tweets.show', ['tweet' => $favorite->id]) }}" class="mt-2 text-gray-500">{{ $favorite->tweet }}</a>
                        </div>
                    </div>
                </div>
            @endforeach
